...
// Custom SVG social icons (lucide doesn't have X/Twitter)
function XIcon() {
  return (
...
const socialLinks = [
  ...
  { label: "X / Twitter", href: brand.social.twitter, icon: <XIcon /> },
];

export default function Footer() {
  return (
    <footer id="contact" className="bg-forest-dark text-bone">
      <div className="max-w-site mx-auto px-4 lg:px-6 py-16 lg:py-20">
        <div className="grid md:grid-cols-2 lg:grid-cols-12 gap-10 xl:gap-8">
            ...
            <div className="mt-6 flex gap-3">
              {socialLinks.map((s) => (
                <a
                  ...
                  className="h-10 w-10 rounded-full border border-bone/20 flex items-center justify-center hover:bg-sage hover:border-sage text-bone/70 hover:text-forest transition-colors"
                  aria-label={s.label}
                >
                  {s.icon}
                </a>
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-2.5">
              {navLinks.map((l) => (
                <li key={l.label}>
                  <Link to={l.href} className="text-sm text-bone/75 hover:text-bone transition-colors">{l.label}</Link>
                </li>
              ))}
            </ul>
          </div>
